 	...
 	function catch_fullscreen_post_page_category_partners() {
        ...
        if ($vl = get_post_custom_values('cr3ativ_sponsorurl'))  {
          $splink = $vl[0];
 				...
 				$output .= '
 					<a class="post-thumbnail" href="' . esc_url( $splink ) . '" title="' . $title_attribute . '">
 						'. $image . '
 					</a>';
        ...
        $output .= '
 				...
 				$output .= the_title( '<header class="entry-header"><h2 class="entry-title"><a href="' . esc_url( $splink ) . '" rel="bookmark">', '</a></h2></header><!-- .entry-header -->', false );
